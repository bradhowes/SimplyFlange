...
class PresetsMenuManager: NSObject {
  ...
  private func getNewPresetName(default: String) -> String? {
    let prompt = NSAlert()
  ...
  private func getRenamePresetName(existing: String) -> String? {
    let prompt = NSAlert()
    prompt.window.delegate = self
...
extension PresetsMenuManager: NSWindowDelegate {
